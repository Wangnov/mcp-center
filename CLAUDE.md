...
- **根目录和每个子目录**（后端、前端、Tauri、测试等）都存在独立的 `CLAUDE.md` 项目记忆文档，执行具体任务前请先阅读对应文件，遵循既定惯例。
- 每次作出可能影响项目记忆的改动（新增文件、组件、流程调整等）后，必须更新相关的一个或多个 `CLAUDE.md` 项目记忆文档；若无需更新，请在总结中说明原因。
